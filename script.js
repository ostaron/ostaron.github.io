...
lcboapp.userLocationTool = () => {
	...
	$("#resultsWrapper").on("click", ".storeEntry", () => {
		
		// I LEFT OFF HERE
		// FUCKY THINGS HAPPEN WITH this
...
$("#pageLinks").on("click", ".prodPrev", function() {
	resultsGet(lcboapp.prevProdPageUrl);
});
$("#pageLinks").on("click", ".prodNext", function() {
	resultsGet(lcboapp.nextProdPageUrl);
});
$('#search').submit(function(evt) {
	...
	if (searchQuery == "") {
		...
		lcboapp.searchUrl = "/products?access_key=MDo3MmM2MDczOC05ZmFhLTExZTYtOWExYS1kYjhlYzc1N2RmMmE6Y0J3VUM1eDlhWFFwUVNWMDdFUUQ4YmdxV2pNOUJFMUpOOUEw&per_page=10&q=" + searchQuery + "&xmlToJSON=false";
		resultsGet(lcboapp.searchUrl);
	}
});
...
$("#pageLinks").on("click", ".storePrev", function() {
	storesGet(lcboapp.prevStorePageUrl);
});
$("#pageLinks").on("click", ".storeNext", function() {
	storesGet(lcboapp.nextStorePageUrl);
});
$("#prodNameStoreList").on("click", function() {
	resultsGet(lcboapp.currentProdPage);
});
